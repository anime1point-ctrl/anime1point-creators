/**
 * CREATORS â Anime1Point Creators Hub
 * Sprint 5.3: Launch-readiness fixes â creator status, flags, channel IDs
 *
 * creatorStatus: "anime1point" | "featured" | "rising"
 * featuredVideoIds: array â first entry shown as hero on profile page
 */
export const CREATORS = [

  // ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  // PLATFORM CREATOR
  // ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  {
      id: 'anime1point',
      name: 'Anime1Point',
      handle: '@anime1point',
      creatorStatus: 'anime1point',
      category: 'anime',
      avatar: 'A1',
      avatarGradient: 'linear-gradient(135deg,#e63946,#7b2fff)',
      bio: "The flagship Anime1Point channel. Focused on That Time I Got Reincarnated as a Slime (Tensura) Season 4 â episode reviews, English Dub reactions, manga breakdowns, character analysis and lore deep dives.",
      tags: ['Tensura Season 4', 'Episode Reviews', 'English Dub', 'Manga Breakdown', 'Lore Deep Dives'],
      featuredVideoIds: ['_4xtVj881w4', 'nyJ_8jVKJKc', 'ommA4DBy5RQ'],
      youtubeUrl: 'https://www.youtube.com/@anime1point',
      youtubeChannelId: 'UCXbs_3dDTMTwjsHGSxDdOAQ',
      status: 'approved',
      featured: true,
  },

  // ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  // FEATURED CREATORS â TIER 1 (Sprint 4 originals)
  // ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  {
      id: 'gigguk',
      name: 'Gigguk',
      handle: '@Gigguk',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'GG',
      avatarGradient: 'linear-gradient(135deg,#ff6b35,#f7c948)',
      bio: "One of YouTube's biggest anime commentators. Known for seasonal nutshells, deep dives into anime culture, and long-form essays that make anime fans see their favourite shows differently.",
      tags: ['Seasonal Reviews', 'Anime Essays', 'Nutshell Series', 'Anime Culture', 'Top Lists'],
      featuredVideoIds: ['lymfyRBPd14', 'rEAB4CKfx-8', 'wnN4BjkVZNo'],
      youtubeUrl: 'https://www.youtube.com/@gigguk',
      youtubeChannelId: 'UC7dF9qfBMXrjNEqHsxNbm0g',
      status: 'approved',
      featured: true,
  },
  {
      id: 'theanimeman',
      name: 'The Anime Man',
      handle: '@TheAnimeMan',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'AM',
      avatarGradient: 'linear-gradient(135deg,#1a1a2e,#e94560)',
      bio: "Half-Japanese creator living in Tokyo with deep insight into anime, manga, and Japanese culture. Combines personal experience with critical analysis across all genres and mediums.",
      tags: ['Anime Reviews', 'Japan Culture', 'Manga', 'Light Novels', 'Industry Deep Dives'],
      featuredVideoIds: ['mTNMEfvM3Ro', 'RAE9b0rIslw', 'Svd8oeuVB7A'],
      youtubeUrl: 'https://www.youtube.com/@TheAnimeMan',
      youtubeChannelId: 'UC_9I_ZACNqJtMlFhIUQGVcg',
      status: 'approved',
      featured: true,
  },
  {
      id: 'mothersbasement',
      name: "Mother's Basement",
      handle: '@mothersbasement',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'MB',
      avatarGradient: 'linear-gradient(135deg,#0f3460,#533483)',
      bio: "Deep analysis of anime openings, endings, and storytelling. Geoff Thew breaks down what makes anime great with technical precision â from sakuga animation to narrative structure and symbolism.",
      tags: ['OP/ED Analysis', 'Anime Analysis', 'Storytelling', 'Sakuga', 'Seasonal Coverage'],
      featuredVideoIds: ['1RRL4Bv3DSM', 'wtMM_sLZFMQ', 'LTwWZPmpoc0'],
      youtubeUrl: 'https://www.youtube.com/@mothersbasement',
      youtubeChannelId: 'UCBs2Y3i14e1NWQxOGliatmg',
      status: 'approved',
      featured: true,
  },
  {
      id: 'glassreflection',
      name: 'Glass Reflection',
          handle: '@GlassReflection',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'GR',
      avatarGradient: 'linear-gradient(135deg,#2193b0,#6dd5ed)',
      bio: "Arkada delivers balanced, spoiler-conscious seasonal anime reviews with over a decade of consistency. Known for watching every seasonal premiere and giving honest recommendations for all types of anime fans.",
      tags: ['Seasonal Reviews', 'Anime Recommendations', 'Spoiler-Free', 'First Impressions', 'Top 10'],
      featuredVideoIds: ['81Xd9uLqIe8', 'J2The5iHuZM', 'YMsPIC_XU8o'],
      youtubeUrl: 'https://www.youtube.com/@GlassReflection',
      youtubeChannelId: 'UCmAFOAkBiGeDGmHOCAwQntA',
      status: 'approved',
      featured: true,
  },
  {
      id: 'ahbrandon',
      name: 'AH Brandon Reviews',
      handle: '@AHBrandonReviews',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'AB',
      avatarGradient: 'linear-gradient(135deg,#11998e,#38ef7d)',
      bio: "Weekly episode-by-episode reactions and reviews covering the biggest seasonal anime. Brandon's genuine enthusiasm and emotional responses make every episode breakdown feel like watching with a friend.",
      tags: ['Episode Reviews', 'Seasonal Anime', 'Reactions', 'Tensura', 'Re:Zero'],
      featuredVideoIds: ['o82v-4ESYyA', 'FcpP1qdbz8k', 'xQXd_TN8724'],
      youtubeUrl: 'https://www.youtube.com/@AHBrandonReviews',
      youtubeChannelId: 'UCqbncBKBFDtFGz_JdEfyopQ',
      status: 'approved',
      featured: true,
  },
  {
      id: 'chibireviewsyt',
      name: 'Chibi Reviews',
      handle: '@ChibiReviews',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'CR',
      avatarGradient: 'linear-gradient(135deg,#f953c6,#b91d73)',
      bio: "Covering the latest anime news, controversies, and industry commentary. Chibi Reviews keeps the anime community informed with fast-paced, opinionated takes on what is happening right now.",
      tags: ['Anime News', 'Industry Commentary', 'Crunchyroll', 'Community', 'Seasonal Updates'],
      featuredVideoIds: ['SeGTseOKl4A', 'Steb4F40jAQ', 'KYXQqwbIIJs'],
      youtubeUrl: 'https://www.youtube.com/@ChibiReviews',
      youtubeChannelId: 'UCsAifHRSMPlDFwMRrT-jFVw',
      status: 'approved',
      featured: true,
  },
  {
      id: 'realliferyan',
      name: 'RealLifeRyan',
      handle: '@RealLifeRyan',
      creatorStatus: 'featured',
      category: 'manga',
      avatar: 'RR',
      avatarGradient: 'linear-gradient(135deg,#373B44,#4286f4)',
      bio: "In-depth manga reading vlogs and chapter reviews. Best known for the Berserk Revisited series â a volume-by-volume journey through one of manga's greatest masterpieces alongside film and pop culture analysis.",
      tags: ['Berserk', 'Manga Reviews', 'Reading Vlogs', 'Chapter Analysis', 'Classic Manga'],
      featuredVideoIds: ['0_Keq4SgSOI', 'Ec9xELfuDwE', 'c2kdmDkXtsc'],
      youtubeUrl: 'https://www.youtube.com/@RealLifeRyan',
      youtubeChannelId: 'UCTbLbMFjEPInxbR0W5NvQFg',
      status: 'approved',
      featured: true,
  },
  {
      id: 'totallynotmark',
      name: 'Totally Not Mark',
      handle: '@TotallyNotMark',
      creatorStatus: 'featured',
      category: 'manga',
      avatar: 'TM',
      avatarGradient: 'linear-gradient(135deg,#f7971e,#ffd200)',
      bio: "Long-form anime and manga reviews with meticulous research and genuine passion. Comprehensive breakdowns of Dragon Ball, JJK, and Chainsaw Man set the standard for manga criticism on YouTube.",
      tags: ['Manga Analysis', 'Dragon Ball', 'Chainsaw Man', 'JJK', 'Long-Form Reviews'],
      featuredVideoIds: ['Rr7-wlQhGKA', 'Sr2xo5dKDkc', 'tLKYatfJOKM'],
      youtubeUrl: 'https://www.youtube.com/@TotallyNotMark',
      youtubeChannelId: 'UCnKCBRMkVa9cGgdFNREJ8bw',
      status: 'approved',
      featured: true,
  },
  {
      id: 'animeballsdeep',
      name: 'Anime Balls Deep',
      handle: '@AnimeBallsDeep',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'BD',
      avatarGradient: 'linear-gradient(135deg,#141E30,#243B55)',
      bio: "The deepest lore dives on YouTube. Covering the complete stories behind Tensura, One Piece, Naruto, and more â including light novel content beyond the anime. Essential viewing for fans who want the full picture.",
      tags: ['Tensura Lore', 'One Piece', 'Story Analysis', 'Light Novel Spoilers', 'Complete Story'],
      featuredVideoIds: ['ov7rqWJlB1s', 'gvjoXIawoRE', 'xGVVM5aE98I'],
      youtubeUrl: 'https://www.youtube.com/@AnimeBallsDeep',
      youtubeChannelId: 'UC_pGPEPpDdD7QwRFAMBF7jQ',
      status: 'approved',
      featured: true,
  },
  {
      id: 'bobsamurai',
      name: 'BobSamurai',
      handle: '@BobSamurai',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'BS',
      avatarGradient: 'linear-gradient(135deg,#4a0000,#cc0000)',
      bio: "Champion of overlooked and classic anime. BobSamurai digs through decades of anime history to surface hidden gems most fans have never heard of â from 1960s classics to underrated 2000s series.",
      tags: ['Hidden Gems', 'Classic Anime', 'Overlooked Series', 'Retro Anime', 'Anime History'],
      featuredVideoIds: ['-FkMyZn5jeE', 'wzNjhsstG4g', 'ToTZ6ssJGT0'],
      youtubeUrl: 'https://www.youtube.com/@BobSamurai',
      youtubeChannelId: 'UCmpkbRBaHGMNrNQy7UqXuUQ',
      status: 'approved',
      featured: false,
  },

  // ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  // FEATURED CREATORS â TIER 2 (Sprint 5.2 additions)
  // ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  {
      id: 'tekking101',
      name: 'Tekking101',
      handle: '@Tekking101',
      creatorStatus: 'featured',
      category: 'manga',
      avatar: 'TK',
      avatarGradient: 'linear-gradient(135deg,#1a1a2e,#e94560)',
      bio: "The go-to channel for One Piece chapter breakdowns and theory content. Tekking101 has covered One Piece weekly for over a decade with infectious energy, deep lore knowledge, and some of the best reaction content in the community.",
      tags: ['One Piece', 'Chapter Reviews', 'Theories', 'Manga Breakdown', 'Weekly Coverage'],
      featuredVideoIds: ['TK_OP_ch1159', 'TK_OP_theory1', 'TK_OP_ch1158'],
      youtubeUrl: 'https://www.youtube.com/@Tekking101',
      youtubeChannelId: 'UCxhThTTBwXQ9FhPAVnPPTkQ',
      status: 'approved',
      featured: true,
  },
  {
      id: 'yaboyRoshi',
      name: 'YaBoyRoshi',
      handle: '@YaBoyRoshi',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'YR',
      avatarGradient: 'linear-gradient(135deg,#fc4a1a,#f7b733)',
      bio: "High-energy episode reactions and breakdowns for the biggest anime of each season. YaBoyRoshi covers Re:Zero, JJK, Bleach, and Dragon Ball with the kind of genuine hype that makes every episode feel like an event.",
      tags: ['Episode Reactions', 'Re:Zero', 'JJK', 'Bleach', 'Dragon Ball'],
      featuredVideoIds: ['YBR_JJK_ep24', 'YBR_RZ_s4ep9', 'YBR_BLEACH_ep28'],
      youtubeUrl: 'https://www.youtube.com/@YaBoyRoshi',
      youtubeChannelId: 'UCHiGZBD4EEVzxQEuxIukByA',
      status: 'approved',
      featured: true,
  },
  {
      id: 'foxenAnime',
      name: 'Foxen Anime',
      handle: '@FoxenAnime',
      creatorStatus: 'featured',
      category: 'light-novel',
      avatar: 'FA',
      avatarGradient: 'linear-gradient(135deg,#f7971e,#ffd200)',
      bio: "The definitive light novel coverage channel. Foxen Anime goes beyond the anime to break down what happens in the source material for Overlord, Mushoku Tensei, Tensura, and more â essential for LN readers and anime fans curious about what comes next.",
      tags: ['Light Novels', 'Overlord', 'Mushoku Tensei', 'LN Spoilers', 'Source Material'],
      featuredVideoIds: ['FA_OVL_vol16', 'FA_MT_vol24', 'FA_OVL_full'],
      youtubeUrl: 'https://www.youtube.com/@FoxenAnime',
      youtubeChannelId: 'UCG-8wzbSmKJfbJmZgewAYWg',
      status: 'approved',
      featured: true,
  },
  {
      id: 'canipa',
      name: 'The Canipa Effect',
      handle: '@TheCanipa',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'CE',
      avatarGradient: 'linear-gradient(135deg,#1c1c2e,#a855f7)',
      bio: "The definitive source for anime production analysis. Callum covers studio pipelines, animation director interviews, pre-production reports, and the stories behind how anime gets made â indispensable for industry enthusiasts.",
      tags: ['Production Analysis', 'Studio Coverage', 'Sakuga', 'Industry', 'Behind The Scenes'],
      featuredVideoIds: ['CE_prod_spring26', 'CE_prod_frieren', 'CE_prod_tensura'],
      youtubeUrl: 'https://www.youtube.com/@TheCanipa',
      youtubeChannelId: 'UCN6KO6tHLiNLBHK2Kj7MSMQ',
      status: 'approved',
      featured: true,
  },
  {
      id: 'sawyerGaming',
      name: 'Sawyer7mage',
      handle: '@sawyer7mage',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'S7',
      avatarGradient: 'linear-gradient(135deg,#141E30,#243B55)',
      bio: "Bleach specialist and Shonen authority. Sawyer7mage provides the deepest episode analysis and lore coverage for Bleach: TYBW, plus comprehensive takes on Dragon Ball, Naruto, and the broader Shonen landscape.",
      tags: ['Bleach', 'TYBW', 'Shonen', 'Episode Analysis', 'Dragon Ball'],
      featuredVideoIds: ['S7_BLEACH_tybw3', 'S7_BLEACH_yhwach', 'S7_BLEACH_bankai'],
      youtubeUrl: 'https://www.youtube.com/@sawyer7mage',
      youtubeChannelId: 'UCMJqJTPjkiW1bLLy8vRXv5A',
      status: 'approved',
      featured: true,
  },
  {
      id: 'nuxTaku',
      name: 'NuxTaku',
      handle: '@NuxTaku',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'NT',
      avatarGradient: 'linear-gradient(135deg,#8e2de2,#4a00e0)',
      bio: "Anime commentary, community events, and cultural deep dives with a comedic edge. NuxTaku covers everything from seasonal highlights to fandom history and internet drama â always entertaining, always opinionated.",
      tags: ['Anime Commentary', 'Community', 'Fandom', 'Cultural Deep Dives', 'Entertainment'],
      featuredVideoIds: ['NT_isekai_debate', 'NT_community'],
      youtubeUrl: 'https://www.youtube.com/@NuxTaku',
      youtubeChannelId: 'UCOjqEDCGFNMhIXXcQE-HVCA',
      status: 'approved',
      featured: true,
  },
  {
      id: 'animeUproar',
      name: 'AnimeUproar',
      handle: '@AnimeUproar',
      creatorStatus: 'featured',
      category: 'anime',
      avatar: 'AU',
      avatarGradient: 'linear-gradient(135deg,#f7971e,#ffd200)',
      bio: "Comprehensive Naruto, Boruto, and Shonen coverage with a focus on power scaling, lore analysis, and chapter-by-chapter breakdowns. AnimeUproar is the channel for fans who want every detail explained.",
      tags: ['Naruto', 'Boruto', 'Power Scaling', 'Shonen', 'Lore Analysis'],
      featuredVideoIds: ['AU_BTV_ch35', 'AU_NAR_power', 'AU_BTV_ch34'],
      youtubeUrl: 'https://www.youtube.com/@AnimeUproar',
      youtubeChannelId: 'UCnuClNCnMnHOxPTcGmOCxEA',
      status: 'approved',
      featured: true,
  },

  // ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  // RISING CREATORS â Sprint 5.2 additions
  // ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
  {
      id: 'mangaRamen',
      name: 'MangaRamen',
      handle: '@MangaRamen',
      creatorStatus: 'rising',
      category: 'manga',
      avatar: 'MR',
      avatarGradient: 'linear-gradient(135deg,#e96c57,#f7b733)',
      bio: "Serious manga criticism for serious manga readers. MangaRamen covers Kingdom, Vagabond, Berserk, and the best of classic and modern manga â bringing a literary perspective to the medium's greatest works.",
      tags: ['Kingdom', 'Vagabond', 'Classic Manga', 'Manga Criticism', 'Long-Form Reviews'],
      featuredVideoIds: ['MR_KINGDOM_ch800', 'MR_VAGABOND_review', 'MR_BERSERK_legacy'],
      youtubeUrl: 'https://www.youtube.com/@MangaRamen',
      youtubeChannelId: 'UCPaYBTaHxUHDvzXOLqm4TOg',
      status: 'approved',
      featured: false,
  },
  {
      id: 'otakuTherapy',
      name: 'Otaku Therapy',
      handle: '@OtakuTherapy',
      creatorStatus: 'rising',
      category: 'manga',
      avatar: 'OT',
      avatarGradient: 'linear-gradient(135deg,#005c97,#363795)',
      bio: "Thoughtful manga analysis focused on storytelling, themes, and artistic merit. Specialises in Frieren, JoJo, and literary manga that reward deep reading â perfect for fans who want more than a plot summary.",
      tags: ['Frieren', 'JoJo', 'Manga Analysis', 'Storytelling', 'Literary Manga'],
      featuredVideoIds: ['OT_FR_ch130', 'OT_FR_full', 'OT_JOJO_SBR'],
      youtubeUrl: 'https://www.youtube.com/@OtakuTherapy',
      youtubeChannelId: 'NEEDS_VERIFICATION',
      status: 'approved',
      featured: false,
  },
  {
      id: 'animaexplained',
      name: 'Anime Explained',
      handle: '@AnimeExplained',
      creatorStatus: 'rising',
      category: 'anime',
      avatar: 'AE',
      avatarGradient: 'linear-gradient(135deg,#1a1a2e,#16213e)',
      bio: "Fast, accessible lore and theory videos covering Naruto, Dragon Ball, Bleach, and beyond. Anime Explained makes complex power systems and story arcs easy to follow for both newcomers and long-term fans.",
      tags: ['Lore', 'Theory', 'Naruto', 'Dragon Ball', 'Bleach'],
      featuredVideoIds: [],
      youtubeUrl: 'https://www.youtube.com/@AnimeExplained',
      youtubeChannelId: 'NEEDS_VERIFICATION',
      status: 'pending',
      featured: false,
  },
  {
      id: 'foxclover',
      name: 'Foxclover',
      handle: '@Foxclover',
      creatorStatus: 'rising',
      category: 'anime',
      avatar: 'FC',
      avatarGradient: 'linear-gradient(135deg,#11998e,#38ef7d)',
      bio: "Emotional, beautifully written essays on slice-of-life and fantasy anime. Foxclover specialises in Frieren, Dungeon Meshi, and anime that prioritise character and atmosphere â a must-follow for fans of thoughtful, reflective content.",
      tags: ['Frieren', 'Slice of Life', 'Anime Essays', 'Fantasy', 'Character Studies'],
      featuredVideoIds: ['FC_FR_elf', 'FC_FR_anime'],
      youtubeUrl: 'https://www.youtube.com/@Foxclover',
      youtubeChannelId: 'NEEDS_VERIFICATION',
      status: 'approved',
      featured: false,
  },
  {
      id: 'isekaiExplained',
      name: 'Isekai Explained',
      handle: '@IsekaiExplained',
      creatorStatus: 'rising',
      category: 'light-novel',
      avatar: 'IE',
      avatarGradient: 'linear-gradient(135deg,#2980b9,#2c3e50)',
      bio: "The light novel specialists. Isekai Explained covers Overlord, Mushoku Tensei, Bookworm, Re:Zero, and the full depth of the LN landscape â going far beyond what the anime adapts to give readers and viewers the complete story.",
      tags: ['Light Novels', 'Isekai', 'Overlord', 'Mushoku Tensei', 'Re:Zero LN'],
      featuredVideoIds: ['IE_OVL_ainz', 'IE_MT_powers', 'IE_REZERO_ln'],
      youtubeUrl: 'https://www.youtube.com/@IsekaiExplained',
      youtubeChannelId: 'NEEDS_VERIFICATION',
      status: 'approved',
      featured: false,
  },
  {
      id: 'animeSenpai',
      name: 'Anime Senpai Clips',
      handle: '@AnimeSenpai',
      creatorStatus: 'rising',
      category: 'anime',
      avatar: 'AS',
      avatarGradient: 'linear-gradient(135deg,#f953c6,#b91d73)',
      bio: "Breaking anime news and seasonal first impressions delivered fast. Anime Senpai covers announcements, trailers, and seasonal previews â the quickest way to stay current with what's coming in anime.",
      tags: ['Anime News', 'First Impressions', 'Seasonal Preview', 'Trailers', 'Announcements'],
      featuredVideoIds: [],
      youtubeUrl: 'https://www.youtube.com/@AnimeSenpai',
      youtubeChannelId: 'NEEDS_VERIFICATION',
      status: 'pending',
      featured: false,
  },
  {
      id: 'pedalfan',
      name: 'PedaFan',
      handle: '@PedaFan',
      creatorStatus: 'rising',
      category: 'anime',
      avatar: 'PF',
      avatarGradient: 'linear-gradient(135deg,#e91e8c,#f7c948)',
      bio: "A rising voice covering shojo, josei, and romance anime with critical depth. PedaFan brings much-needed balance to the anime discourse â exploring stories, art, and emotional beats that too many channels overlook.",
      tags: ['Shojo', 'Josei', 'Romance Anime', 'Anime Reviews', 'Underrepresented Genres'],
      featuredVideoIds: [],
      youtubeUrl: 'https://www.youtube.com/@PedaFan',
      youtubeChannelId: 'NEEDS_VERIFICATION',
      status: 'pending',
      featured: false,
  },
  
// ════════════════════════════════════════════════════════════
// SPRINT 5.4 — LIGHT NOVEL CREATOR ADDITIONS
// AniNews, Echidnut, Hiding in Public, Manime Matt
// ════════════════════════════════════════════════════════════
{
id: 'aninews',
name: 'AniNews',
handle: '@AniNews',
creatorStatus: 'rising',
category: 'light-novel',
avatar: 'AN',
avatarGradient: 'linear-gradient(135deg,#0f2027,#203a43,#2c5364)',
bio: 'Deep-dive light novel coverage for Overlord, Re:Zero, Mushoku Tensei, and Tensura. AniNews regularly covers LN cut content, novel-only material, timelines, and character analysis that goes far beyond the anime adaptation.',
tags: ['Light Novels', 'Overlord', 'Re:Zero', 'Mushoku Tensei', 'LN Cut Content'],
featuredVideoIds: ['LN002', 'LN004', 'LN021'],
youtubeUrl: 'https://www.youtube.com/@AniNews',
youtubeChannelId: 'NEEDS_VERIFICATION',
status: 'approved',
featured: false,
},
{
id: 'echidnut',
name: 'Echidnut',
handle: '@Echidnut',
creatorStatus: 'rising',
category: 'light-novel',
avatar: 'EC',
avatarGradient: 'linear-gradient(135deg,#1a1a2e,#c94b4b)',
bio: 'The go-to channel for Re:Zero lore, timelines, and deep character analysis. Echidnut specialises in the Re:Zero light novel universe — covering every death, route, and story detail far beyond what the anime shows.',
tags: ['Re:Zero', 'LN Lore', 'Character Analysis', 'Timelines', 'Re:Zero Spoilers'],
featuredVideoIds: ['LN003', 'LN013', 'LN032'],
youtubeUrl: 'https://www.youtube.com/@Echidnut',
youtubeChannelId: 'NEEDS_VERIFICATION',
status: 'approved',
featured: false,
},
{
id: 'hidingInPublic',
name: 'Hiding in Public',
handle: '@HidinginPublic',
creatorStatus: 'rising',
category: 'light-novel',
avatar: 'HP',
avatarGradient: 'linear-gradient(135deg,#2c3e50,#3498db)',
bio: 'Deep character breakdowns and psychological analysis of light novel protagonists. Best known for Classroom of the Elite and Eminence in Shadow — bringing a philosophical and analytical lens to the most complex LN characters.',
tags: ['Character Analysis', 'CotE', 'Eminence in Shadow', 'Philosophy', 'LN Deep Dives'],
featuredVideoIds: ['LN010', 'LN015', 'LN025'],
youtubeUrl: 'https://www.youtube.com/@HidinginPublic',
youtubeChannelId: 'NEEDS_VERIFICATION',
status: 'approved',
featured: false,
},
{
id: 'manimeMatt',
name: 'Manime Matt',
handle: '@ManimeMatt',
creatorStatus: 'rising',
category: 'light-novel',
avatar: 'MM',
avatarGradient: 'linear-gradient(135deg,#373B44,#4286f4)',
bio: 'Light novel reviews and reading recommendations for every type of reader. Manime Matt covers hidden gems, dark novels, and titles that deserve far more attention — essential viewing for anyone looking to start or expand their LN library.',
tags: ['LN Reviews', 'Recommendations', 'Hidden Gems', 'Dark Light Novels', 'Reading Lists'],
featuredVideoIds: ['LN042', 'LN045', 'LN050'],
youtubeUrl: 'https://www.youtube.com/@ManimeMatt',
youtubeChannelId: 'NEEDS_VERIFICATION',
status: 'approved',
featured: false,
},
]

export const APPROVED_CREATORS = CREATORS.filter(c => c.status === 'approved')
export const FEATURED_CREATORS = CREATORS.filter(c => c.featured)

// ════════════════════════════════════════════════════════════
// SPRINT 5.6 — SEINEN, ECCHI & DRAMA CREATOR ADDITIONS
// New voices covering mature genres across anime, manga, light-novel
// ════════════════════════════════════════════════════════════
{
    id: 'danielGreene',
        name: 'Daniel Greene',
        handle: '@DanielGreeneReviews',
        creatorStatus: 'rising',
        category: 'anime',
        avatar: 'DG',
        avatarGradient: 'linear-gradient(135deg,#1a1a1a,#c0392b)',
        bio: 'Thoughtful anime and manga reviews with a focus on mature storytelling. Daniel Greene digs into classic seinen titles like Berserk, Parasyte, and Fullmetal Alchemist — delivering balanced takes that fans of literary manga will appreciate.',
        tags: ['Seinen', 'Manga Reviews', 'Berserk', 'FMA', 'Parasyte'],
        featuredVideoIds: ['H1B1drKq1wI', 'nHtA0nuIH-o', '_b4fw4QTiY0'],
        youtubeUrl: 'https://www.youtube.com/@DanielGreeneReviews',
        youtubeChannelId: 'NEEDS_VERIFICATION',
        status: 'approved',
        featured: false,
      },
    {
        id: 'mudanTV',
        name: 'MudanTV',
        handle: '@MudanTV',
        creatorStatus: 'rising',
        category: 'anime',
        avatar: 'MT',
        avatarGradient: 'linear-gradient(135deg,#2c3e50,#e74c3c)',
        bio: 'Quick-fire, honest anime reviews covering both mainstream and overlooked titles. MudanTV is known for punchy "Watch or Not" verdicts on drama and seinen series like Banana Fish, giving newcomers a fast guide to the best mature anime.',
        tags: ['Anime Reviews', 'Drama', 'Seinen', 'Banana Fish', 'Watch or Not'],
        featuredVideoIds: ['kssXuu0J8XQ', 'zF-BQh9YZjo'],
        youtubeUrl: 'https://www.youtube.com/@MudanTV',
        youtubeChannelId: 'NEEDS_VERIFICATION',
        status: 'approved',
        featured: false,
    },
    {
        id: 'seinenMosaic',
        name: 'Seinen Mosaic',
        handle: '@SeinenMosaic',
        creatorStatus: 'rising',
        category: 'manga',
        avatar: 'SM',
        avatarGradient: 'linear-gradient(135deg,#34495e,#9b59b6)',
        bio: 'Dedicated seinen manga coverage for readers who want more than mainstream picks. Seinen Mosaic surfaces underrated titles across action, drama, and psychological genres — a go-to for manga fans seeking their next deep read.',
        tags: ['Seinen Manga', 'Underrated', 'Manga Reviews', 'Recommendations', 'Hidden Gems'],
        featuredVideoIds: ['vHesm8i2c94', 'I78K8lDKAnI'],
        youtubeUrl: 'https://www.youtube.com/@SeinenMosaic',
        youtubeChannelId: 'NEEDS_VERIFICATION',
        status: 'approved',
        featured: false,
    },
    {
        id: 'delusional675',
        name: 'Delusional',
        handle: '@Delusional675',
        creatorStatus: 'rising',
        category: 'manga',
        avatar: 'DL',
        avatarGradient: 'linear-gradient(135deg,#2d3436,#6c5ce7)',
        bio: 'Manga recommendation videos with a sharp eye for completed seinen gems. Delusional curates lists of finished series worth binging, covering psychological thrillers, dark drama, and classic seinen titles that deserve a wider audience.',
        tags: ['Seinen Manga', 'Completed Manga', 'Recommendations', 'Dark Manga', 'Hidden Gems'],
        featuredVideoIds: ['59wLn_COfoo'],
        youtubeUrl: 'https://www.youtube.com/@Delusional675',
        youtubeChannelId: 'NEEDS_VERIFICATION',
        status: 'approved',
        featured: false,
    },
    {
        id: 'belmakor19',
        name: 'Belmakor19',
        handle: '@Belmakor19',
        creatorStatus: 'rising',
        category: 'light-novel',
        avatar: 'BK',
        avatarGradient: 'linear-gradient(135deg,#1e3799,#0c2461)',
        bio: 'Dedicated light novel reviews with a focus on dark fantasy, seinen, and ecchi LN titles. Belmakor19 covers volumes other channels skip — from Goblin Slayer to mature isekai — giving LN readers detailed breakdowns of source material.',
        tags: ['Light Novel Reviews', 'Goblin Slayer', 'Dark Fantasy', 'Seinen LN', 'Volume Reviews'],
        featuredVideoIds: ['QceeYYpGNZ8'],
        youtubeUrl: 'https://www.youtube.com/@Belmakor19',
        youtubeChannelId: 'NEEDS_VERIFICATION',
        status: 'approved',
        featured: false,
    },
    {
        id: 'otakuRR',
        name: 'Otaku R&R',
        handle: '@otakurr',
        creatorStatus: 'rising',
        category: 'light-novel',
        avatar: 'OR',
        avatarGradient: 'linear-gradient(135deg,#6a1a4c,#c0392b)',
        bio: 'Light novel and manga reviews covering ecchi, harem, and mature genres with a collector\'s perspective. Otaku R&R bridges the gap between LN readers and anime fans, covering Yen Press and other licensed releases.',
        tags: ['Light Novels', 'Ecchi LN', 'Harem', 'Yen Press', 'Collector Reviews'],
        featuredVideoIds: ['83rhfB3Iq8Y'],
        youtubeUrl: 'https://www.youtube.com/@otakurr',
        youtubeChannelId: 'NEEDS_VERIFICATION',
        status: 'approved',
        featured: false,
    },
    {
        id: 'scamboliReviews',
        name: 'Scamboli Reviews',
        handle: '@ScamboliReviews',
        creatorStatus: 'rising',
        category: 'anime',
        avatar: 'SR',
        avatarGradient: 'linear-gradient(135deg,#f39c12,#e74c3c)',
        bio: 'Energetic anime reviews spanning every genre — from ecchi guilty pleasures to gripping seinen dramas. Scamboli Reviews pairs entertaining delivery with genuine critical insight, covering the full spectrum of anime with honest opinions and sharp humor.',
        tags: ['Anime Reviews', 'Ecchi', 'Seinen', 'Honest Reviews', 'All Genres'],
        featuredVideoIds: ['9HC13nBIS7g', 'ogW7xRRxK0w'],
        youtubeUrl: 'https://www.youtube.com/@ScamboliReviews',
        youtubeChannelId: 'NEEDS_VERIFICATION',
        status: 'approved',
        featured: false,
    },
    {
        id: 'jaranime',
        name: 'JarAnime',
        handle: '@jaranime0',
        creatorStatus: 'rising',
        category: 'anime',
        avatar: 'JA',
        avatarGradient: 'linear-gradient(135deg,#00b894,#2d3436)',
        bio: 'Long-form anime storytelling recaps and analysis, specialising in drama and thriller series. JarAnime distills complex narratives into deeply satisfying breakdowns — ideal for viewers who want the full emotional arc of a series explained.',
        tags: ['Anime Recap', 'Drama', 'Thriller', 'Story Analysis', 'Long-Form'],
        featuredVideoIds: ['SHZWDsPs7Qw'],
        youtubeUrl: 'https://www.youtube.com/@jaranime0',
        youtubeChannelId: 'NEEDS_VERIFICATION',
        status: 'approved',
        featured: false,
    },
    {
        id: 'hyacinthReviews',
        name: 'Hyacinth Reviews',
        handle: '@matt_sun',
        creatorStatus: 'rising',
        category: 'anime',
        avatar: 'HR',
        avatarGradient: 'linear-gradient(135deg,#6c3483,#1a5276)',
        bio: 'Thoughtful video essays on why seinen anime deserves a bigger audience. Hyacinth Reviews builds compelling arguments for mature anime like Vinland Saga and Berserk, helping action fans discover the depth hidden in adult-oriented storytelling.',
        tags: ['Seinen Anime', 'Video Essays', 'Vinland Saga', 'Berserk', 'Mature Anime'],
        featuredVideoIds: ['UY_6d79zNcU'],
        youtubeUrl: 'https://www.youtube.com/@matt_sun',
        youtubeChannelId: 'NEEDS_VERIFICATION',
        status: 'approved',
        featured: false,
    },
    {
        id: 'myAnimeEncyclopedia',
        name: 'My Anime Encyclopedia',
        handle: '@myanimeencyclopedia',
        creatorStatus: 'rising',
        category: 'anime',
        avatar: 'ME',
        avatarGradient: 'linear-gradient(135deg,#16a085,#1a5276)',
        bio: 'Structured spoiler-free anime reviews covering plot, characters, and overall verdict. My Anime Encyclopedia covers seinen and drama series with clear, accessible ratings — a reliable guide for fans deciding what to watch next.',
        tags: ['Anime Reviews', 'Spoiler-Free', 'Seinen', 'Drama', 'Ratings'],
        featuredVideoIds: ['AlSjlg1dWWE'],
        youtubeUrl: 'https://www.youtube.com/@myanimeencyclopedia',
        youtubeChannelId: 'NEEDS_VERIFICATION',
        status: 'approved',
        featured: false,
    },
export const RISING_CREATORS = CREATORS.filter(c => c.creatorStatus === 'rising')
export const CREATOR_MAP = Object.fromEntries(CREATORS.map(c => [c.id, c]))
