      ...
      <header className="sticky top-0 z-50 bg-bg-card border-b border-border-dim">
        <div className="max-w-7xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <a href="/" target="_blank" rel="noopener noreferrer" className="text-text-secondary text-xs hover:text-text-primary transition-colors">
              View Site
            </a>
            <button onClick={handleLogout} className="text-xs px-3 py-1.5 rounded-lg border border-accent/30 text-accent hover:bg-accent/10 transition-colors">
              Logout
            </button>
          </div>
        </div>
        ...
        <aside className="w-48 shrink-0 hidden md:block">
          <nav className="flex flex-col gap-1">
            {ADMIN_NAV.map(({ to, label }) => (
              <NavLink key={to} to={to} className={lc}>{label}</NavLink>
            ))}
          </nav>
        </aside>
